...
class CandidatePreparationMigrationTests(unittest.TestCase):
    def test_fresh_v21_and_v20_upgrade(self):
        ...
        self.assertEqual(fresh.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 23)
        ...
        self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 23)
        fresh.close()
        upgrade.close()
