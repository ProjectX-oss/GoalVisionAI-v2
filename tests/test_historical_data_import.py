...
class HistoricalMigrationAndStartupTests(unittest.TestCase):
    def test_fresh_v23_schema_has_tables_indexes_and_eight_immutable_guards(self):
        ...
        self.assertEqual(database.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 41)
        objects = {
            (row[0], row[1])
    ...
    def test_v22_to_v23_upgrade_preserves_existing_data(self):
        ...
        self.assertEqual(connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 41)
