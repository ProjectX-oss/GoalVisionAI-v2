...
class ForwardTestFoundationTests(unittest.TestCase):
    ...
    def test_schema_37_fresh_migration(self):
        self.assertEqual(self.db.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 40)
    ...
    def test_v35_upgrade_migration(self):
        ...
        MigrationManager(upgrade.connection).migrate(); self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 40); self.assertEqual(upgrade.connection.execute("PRAGMA foreign_key_check").fetchall(), []); upgrade.close()
    ...
    def test_cli_human_json_diagnose_and_no_startup_execution(self):
        ...
        value = json.loads(out.getvalue()); self.assertEqual(code, 0); self.assertEqual(value["schema_version"], 40); self.assertFalse(value["startup_execution"]); self.assertFalse(value["telegram_transport_constructed"])
