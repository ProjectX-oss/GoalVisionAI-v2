...
class HistoricalTrainingInspectionMigrationStartupTests(unittest.TestCase):
    ...
    def test_fresh_v24_and_v23_upgrade_preserve_data(self):
        ...
        self.assertEqual(fresh.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 26)
        ...
        self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 26)
