...
class ForwardTestMonitoringTests(unittest.TestCase):
    ...
    def test_schema_40_is_latest_and_foreign_keys_hold(self):
        self.assertEqual(max(m.version for m in MIGRATIONS),40); self.assertEqual(self.db.connection.execute("select max(version) from schema_migrations").fetchone()[0],40); self.assertEqual(self.db.connection.execute("pragma foreign_key_check").fetchall(),[])
