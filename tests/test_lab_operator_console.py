...
class LabOperatorConsoleTests(unittest.TestCase):
    ...
    def test_031_schema_40_fresh_upgrade_foreign_keys_and_append_only(self):
        self.assertEqual(max(m.version for m in MIGRATIONS),42);db=Database(str(self.path));self.assertEqual(db.connection.execute("select max(version) from schema_migrations").fetchone()[0],42);self.assertEqual(db.connection.execute("pragma foreign_key_check").fetchall(),[])
