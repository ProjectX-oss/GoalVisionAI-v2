...
class OfficialPredictionPipelineMigrationTests(unittest.TestCase):
    def test_fresh_v22_schema_and_append_only_guards(self):
        ...
        try:
            ...
            self.assertEqual(versions, tuple(range(1, 27)))
            ...
            self.assertEqual(MIGRATIONS[-1].version, 26)
        finally:
            database.close()
    ...
    def test_v21_to_v22_upgrade(self):
        ...
        try:
            ...
            MigrationManager(database.connection).migrate()
            self.assertEqual(database.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 26)
        finally:
            module.MIGRATIONS = original
            database.close()
