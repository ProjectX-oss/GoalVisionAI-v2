...
class OfficialPredictionPipelineMigrationTests(unittest.TestCase):
    def test_fresh_v22_schema_and_append_only_guards(self):
        ...
        try:
            ...
            self.assertEqual(versions, tuple(range(1, 29)))
            ...
            self.assertEqual(MIGRATIONS[-1].version, 28)
        finally:
            database.close()
    ...
    def test_v21_to_v22_upgrade(self):
        ...
        try:
            ...
            MigrationManager(database.connection).migrate()
            self.assertEqual(database.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 28)
        finally:
            module.MIGRATIONS = original
            database.close()
