...
class FirstLabOperationalReadinessTests(unittest.TestCase):
    ...
    def test_22_schema_37_and_zero_startup_execution(self):
        self.assertEqual(self.db.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 37); self.assertEqual(self.db.connection.execute("SELECT COUNT(*) FROM first_lab_run_executions").fetchone()[0], 0)
