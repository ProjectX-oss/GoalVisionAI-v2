...
class MatchDataSnapshotMigrationAndConcurrencyTests(unittest.TestCase):
    def test_fresh_v15_and_v14_upgrade(self):
        ...
        self.assertEqual(fresh.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 18)
        fresh.close()
        ...
        self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 18)
