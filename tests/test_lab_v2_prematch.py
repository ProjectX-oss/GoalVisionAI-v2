...
from app.lab_v2_shadow.forward_evidence import capture_selection, settle_forward
...
@pytest.mark.parametrize('bad', ['stale', 'started', 'nonpositive'])
...
def test_nonpublished_shadow_capture_and_night_settlement(tmp_path):
    repo = ShadowEvidenceRepository(tmp_path/'shadow.db')
    row = {**_controlled_ready_candidate(NOW), 'stage':'EARLY_CANDIDATE',
           'league_id':1,'competition_profile':'SENIOR_MEN_PRO','candidate_lane':'EXPERIMENTAL',
           'profile_policy_version':'LAB_PREMATCH_LIGHT_SAFETY_V2','model_generation':'synthetic-generation'}
    assert capture_selection(repo,row,now=NOW)
    assert not capture_selection(repo,{**row,'captured_odds':'3'},now=NOW+timedelta(minutes=1))
    night = NOW.replace(hour=23)
    class Results:
        calls = 0
        async def fixture(self, identity):
            self.calls += 1
            return {'response':[{'fixture':{'id':identity,'status':{'short':'FT'}},
                                 'score':{'fulltime':{'home':2,'away':0}}}]}
    client = Results()
    assert asyncio.run(settle_forward(repo,client,now=night))['shadow_results_captured']==1
    assert asyncio.run(settle_forward(repo,client,now=night))['shadow_results_captured']==0
    assert client.calls==1
    assert repo.all('forward_result')[0]['outcome']=='WON'
    observation=repo.all('forward_selection')[0]
    assert observation['model_generation']=='synthetic-generation'
    assert observation['accounting']=='SHADOW_LEARNING_ONLY'
    assert not repo.all('publication_cycle')
    assert not repo.all('single_prediction') and not repo.all('bankroll')
    repo.close()
...
def test_shadow_result_source_crash_recovery(tmp_path):
    from app.lab_combo.settlement import resolve_leg
    repo = ShadowEvidenceRepository(tmp_path/'shadow.db')
    row = {**_controlled_ready_candidate(NOW), 'stage':'TRACKING',
           'league_id':1,'competition_profile':'SENIOR_MEN_PRO','candidate_lane':'TRACKING',
           'profile_policy_version':'LAB_PREMATCH_LIGHT_SAFETY_V2','model_generation':'test'}
    assert capture_selection(repo,row,now=NOW)
    frozen = repo.all('forward_selection')[0]
    later = NOW + timedelta(hours=3)
    payload = {'response':[{'fixture':{'id':row['fixture_id'],'status':{'short':'FT'}},
                            'score':{'fulltime':{'home':2,'away':0}}}]}
    source = resolve_leg({**frozen,'observation_id':frozen['selection_key'],'odds':frozen['captured_odds']}, payload, later)
    repo.append('forward_result_source',frozen['selection_key'],source,created_at=later)
    class Forbidden:
        async def fixture(self, identity):
            pytest.fail('Frozen result should recover without provider access')
    result = asyncio.run(settle_forward(repo,Forbidden(),now=later+timedelta(minutes=30)))
    assert result == {'fixtures_requested':0,'shadow_results_captured':1}
    assert repo.all('forward_result_source')==[source]
    repo.close()
...
def test_priority_retry_bound_rotates_after_restart(tmp_path, monkeypatch):
    ...
    for offset in (0,30):
        ...
        retries = report['odds_pagination']['priority_exact_retries']
        assert len(retries)==PRIORITY_EXACT_RETRIES_PER_CYCLE
...
@pytest.mark.parametrize('payload', [{'response':[None]}, {'response':[{'fixture':3}]}, {'errors':{'provider':'down'}}])
def test_shadow_malformed_results_remain_pending(tmp_path, payload):
    repo = ShadowEvidenceRepository(tmp_path/'shadow.db')
    row = {**_controlled_ready_candidate(NOW), 'stage':'EARLY_CANDIDATE',
           'league_id':1,'competition_profile':'SENIOR_MEN_PRO','candidate_lane':'EXPERIMENTAL',
           'profile_policy_version':'LAB_PREMATCH_LIGHT_SAFETY_V2'}
    assert capture_selection(repo,row,now=NOW)
    class Malformed:
        async def fixture(self, identity):
            return payload
    report = asyncio.run(settle_forward(repo,Malformed(),now=NOW+timedelta(hours=3)))
    assert report['shadow_results_captured']==0 and not repo.all('forward_result')
    repo.close()
