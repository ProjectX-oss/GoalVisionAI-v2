...
class PredictionInferenceMigrationTests(unittest.TestCase):
    def test_fresh_v17_and_v16_upgrade(self) -> None:
        ...
        self.assertEqual(fresh.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 19)
        ...
        self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 19)
