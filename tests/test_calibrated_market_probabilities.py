...
class CalibratedMarketProbabilityMigrationTests(unittest.TestCase):
    def test_fresh_v18_and_v17_upgrade(self) -> None:
        ...
        self.assertEqual(fresh.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 24)
        fresh.close()
        ...
        self.assertEqual(upgrade.connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 24)
