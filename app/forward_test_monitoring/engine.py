...
class MonitoringService:
    ...
    def _explainability(self, cutoff: datetime, start: datetime | None) -> dict[str, Any]:
        exists=self.connection.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='prediction_reasoning_records'").fetchone()
        if not exists:return {"reasoning_records_created":0,"sample_status":"FORWARD_TEST_SAMPLE_INSUFFICIENT"}
        clause="r.created_at_utc<=?";params=[cutoff.isoformat()]
        if start is not None:clause+=" AND r.created_at_utc>=?";params.append(start.isoformat())
        rows=self.connection.execute(f"SELECT r.reasoning_json,a.status FROM prediction_reasoning_records r LEFT JOIN prediction_reasoning_audits a ON a.reasoning_id=r.reasoning_id WHERE {clause} ORDER BY r.created_at_utc,r.reasoning_id",tuple(params)).fetchall()
        records=[json.loads(row[0]) for row in rows];statuses=Counter((row[1] or "AUDIT_MISSING") for row in rows);total=len(rows)
        groups=Counter(f["group_id"] for value in records for f in value.get("supporting_factors",()));opposing=Counter(f["group_id"] for value in records for f in value.get("opposing_factors",()));stability=Counter(value.get("explanation_stability_status","UNKNOWN") for value in records)
        return {"reasoning_records_created":total,"valid_reasoning":sum((row[1] or "") in {"REASONING_AUDIT_PASSED","REASONING_AUDIT_WARNING"} for row in rows),"audit_status_counts":dict(sorted(statuses.items())),"audit_pass_rate":str(Decimal(statuses.get("REASONING_AUDIT_PASSED",0))/Decimal(total)) if total else None,"average_public_factor_count":str(Decimal(sum(len(x.get("supporting_factors",())) for x in records))/Decimal(total)) if total else None,"average_risk_factor_count":str(Decimal(sum(len(x.get("risk_factors",())) for x in records))/Decimal(total)) if total else None,"missing_data_disclosure_rate":str(Decimal(sum(bool(x.get("missing_data_disclosures")) for x in records))/Decimal(total)) if total else None,"calibration_warning_disclosure_rate":str(Decimal(sum("nav pietiekama" in x.get("calibration_explanation","") or "būtiski" in x.get("calibration_explanation","") for x in records))/Decimal(total)) if total else None,"shift_warning_disclosure_rate":str(Decimal(sum("nobīde" in x.get("shift_explanation","") for x in records))/Decimal(total)) if total else None,"stability_distribution":dict(sorted(stability.items())),"top_supporting_groups":groups.most_common(10),"top_opposing_groups":opposing.most_common(10),"sample_status":"FORWARD_TEST_SAMPLE_INSUFFICIENT"}
