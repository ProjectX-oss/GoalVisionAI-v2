...
The model generation is explicitly the local deterministic ensemble/policy
version, not an invented champion-registry generation. This checkout has no
adaptive observer or daily adaptive research scheduler package to wire into;
no second learning system or activation path was created.

`settle-shadow` reuses the existing Lab regulation-time `resolve_leg` rules,
fetches one due exact result per fixture, and settles all its shadow markets.
It is allowed at night, uses no odds/history/discovery, and sends no Telegram.
Full result-source evidence is append-only; a crash between source capture and
outcome capture recovers from that source without a new provider request.
Neither observations nor outcomes touch SINGLE/COMBO publication statistics,
Official state or bankroll transactions. The result-only command must be
included in a future authorized deployment's result-worker schedule; this task
does not install timers or modify existing settlement units.
...
.venv/bin/python -m app.lab_v2_shadow settle-shadow --max-calls 20
```
